...
public class VeBigPurpleMushroomFeature extends Feature<BigMushroomFeatureConfig>
{
	public VeBigPurpleMushroomFeature(Function<Dynamic<?>, ? extends BigMushroomFeatureConfig> p_i49863_1_) {
		super(p_i49863_1_);
	}
	...
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, BigMushroomFeatureConfig config) {
		...
		if (j >= 1 && j + i + 1 < 256) {
			...
			else
			{
				...
				BlockState blockstate1 = VeBlocks.purple_mushroom_block.getDefaultState().with(HugeMushroomBlock.DOWN, Boolean.valueOf(false));
